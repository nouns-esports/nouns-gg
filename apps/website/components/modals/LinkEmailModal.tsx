...
export default function LinkEmailModal(props: {
	hasEmail: boolean;
}) {
	...
	const [canReceiveEmails, setCanReceiveEmails] = useState(true);
	...
	const { sendCode, loginWithCode, state } = useLoginWithEmail({
		onComplete: async () => {
			...
			if (canReceiveEmails) {
				const result = await updateNexusAction.executeAsync({
					canRecieveEmails: canReceiveEmails,
				});
		...
		>
			<div className="flex justify-between items-center">
				<p className="text-white text-2xl font-bebas-neue leading-none">
					{verifying ? "Enter Confirmation Code" : "Email"}
				</p>
				<ToggleModal
					id="link-email"
			...
			{!verifying ? (
				<div className="flex gap-2">
					...
					<p className="text-sm text-white">
						I want to receive promotional emails, rewards, and updates from
						Nouns
					</p>
				</div>
			) : null}
			{props.hasEmail ? (
				<button
					disabled={updateNexusAction.isPending}
					onClick={async () => {
						if (updateNexusAction.isPending) return;
